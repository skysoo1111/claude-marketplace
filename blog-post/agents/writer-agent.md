...
layout: post
title: "포스트 제목"
description: "본문 전체를 한 문장으로 요약한 카드용 요약문"
date: ${DATETIME}
categories: [카테고리]
tags: [태그1, 태그2, 태그3]
image:
...
#### description(카드 요약) 규칙 (필수)
- 홈 화면 카드의 제목 아래에 노출되는 한 줄 요약이다. 테마는 `description`을 우선 사용하고, 없으면 본문 첫 문단으로 폴백하므로 **반드시 채운다.**
- **본문 첫 문장을 그대로 복사하지 않는다.** 포스트 전체 내용을 압축한 새 요약 문장을 작성한다.
- 한국어, **한 문장(80~120자)**, 마침표로 끝맺는다. 줄바꿈 금지.
- 핵심 키워드(주제·기술명)를 포함해 검색/카드에서 식별되도록 한다.
- 예) `Kotlin WebFlux에서 HTTP 요청이 처리되는 전체 흐름과, Reactor와 코루틴을 잇는 코루틴 어댑터의 역할을 코드 예제로 정리한다.`
